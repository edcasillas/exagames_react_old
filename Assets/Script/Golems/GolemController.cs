			...
			if (!isDead) {
				...
				if (player.Health > 0) {
					if (Vector3.Distance(player.transform.position, transform.position) < maxCloseDistance &&
					    canAttack && (actualState == GolemStates.Idle || actualState == GolemStates.Walking)) {
					...
					} else if (player.Health > 0 && IsChasing &&
					           Vector3.Distance(player.transform.position, transform.position) > maxCloseDistance &&
					           (!specialAttackTriggered || !canAttack) &&
					           (actualState == GolemStates.Idle || actualState == GolemStates.Walking) &&
					           !takingDamage) {
						Walk();
					}
